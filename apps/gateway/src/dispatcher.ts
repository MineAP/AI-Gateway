...
import type { ProviderAdapter, ProviderRegistry } from "@ai-gateway/provider";
...
export class GatewayRequestDispatcher {
  ...
  async dispatch(request: unknown, options: DispatchOptions): Promise<unknown> {
    ...
    const compatibleRequest = await this.pipeline.processRequest(
      inboundRequest,
      context,
    );
    const providerRequest = await outboundAdapter.buildRequest(
      compatibleRequest,
      context,
    );
    const providerResponse = await this.executor.execute(
      providerRequest,
      outboundAdapter,
      context,
    );
    const outboundResponse = await outboundAdapter.parseResponse(
      providerResponse,
      context,
    );
    const compatibleResponse = await this.pipeline.processResponse(
      outboundResponse,
      context,
    );
